...
if [[ -f "$release_env" ]]; then
  # Generated by the local release tool and included unchanged in Compose bundles.
  # shellcheck disable=SC1090
  source "$release_env"
fi

version=${requested_version:-${WELL_AMBIENT_VERSION:-}}
release_batch=$(printf '%s' "${WELL_AMBIENT_RELEASE_BATCH:-automated-release}" | tr '\r\n\t' '   ' | sed -E 's/[[:space:]]+/ /g; s/^ //; s/ $//')
release_batch=${release_batch:-automated-release}
build_time=${WELL_AMBIENT_BUILD_TIME:-unknown}

if [[ -z "$version" || "$version" == "latest" ]]; then
  echo "release metadata is missing; run make release-metadata or pass an immutable version" >&2
  exit 2
fi
...
record_release_state() {
  ...
  if [[ -f "$release_env" ]]; then
    cp "$release_env" "$state_dir/current-release.env"
  else
    {
  ...
  if [[ -f "$release_notes" ]]; then
    cp "$release_notes" "$state_dir/current-release-notes.txt"
  else
    printf 'Version: %s\nBuild time: %s\nBatch: %s\n' \
...
if [[ "$database_driver" == "setup" ]]; then
  ...
  if [[ -n "$setup_token" ]]; then
    ...
    setup_token_notice=$("${compose[@]}" logs --no-color --tail 100 server 2>&1 | sed -n '/Generated one-time database setup token:/p;/Database setup token saved to /p' | tail -n 2)
    if [[ -z "$setup_token_notice" ]]; then
      echo "automatic setup token was not found in server logs; inspect: docker compose --env-file deploy/.env.production logs server" >&2
      exit 1
    fi
    echo "the program generated this one-time setup token and temporary file:"
    printf '%s\n' "$setup_token_notice"
    echo "open http://127.0.0.1:$http_port and use the generated token shown above"
  fi
